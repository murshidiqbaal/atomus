...
void main() {
  ...
  test('Verify merging duplicate exam sessions and subject marks from admin/teacher uploads', () {
    final mockDbData = [
      ...
      {
        'exam_id': 'exam-2',
        'exams': {'name': 'midterm exam', 'exam_date': '2026-05-15'}, // case-insensitive name
      ...
      // Upload 3: Admin uploads Science again for same "Midterm Exam" (duplicate subject upload), but with a lower score
      {
        ...
        'marks_obtained': 70, // lower score, should be discarded in favor of 90
        'total_marks': 100,
        'grade': 'B',
      },
    ...
    final List<ExamSession> mergedSessions = mergedMarksByExamName.entries.map((entry) {
      return ExamSession.fromMap(examDetailsByExamName[entry.key]!, entry.value);
    }).toList();

    // Verify merging results
    expect(mergedSessions.length, 1); // 2 different exam IDs representing "Midterm Exam" were merged into 1
    ...
    final scienceMark = midterm.subjects.firstWhere((s) => s.subject.toLowerCase() == 'science');
    expect(scienceMark.marksObtained, 95);

    // Verify Mathematics mark (80)
    final mathMark = midterm.subjects.firstWhere((s) => s.subject.toLowerCase() == 'mathematics');
    expect(mathMark.marksObtained, 80);

    // Verify English mark (85)
    final englishMark = midterm.subjects.firstWhere((s) => s.subject.toLowerCase() == 'english');
    ...
    for (final s in midterm.subjects) {
      print('  - ${s.subject}: ${s.marksObtained}/${s.totalMarks} (${s.grade})');
    }
  });
}
